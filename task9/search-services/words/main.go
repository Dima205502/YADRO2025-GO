...
const phraseSizeLimit = 1 * 1000000

type Config struct {
	Address string `yaml:"words_address" env:"WORDS_ADDRESS" env-default:"8081"`
}
...
func (s *server) Norm(_ context.Context, in *wordspb.WordsRequest) (*wordspb.WordsReply, error) {
	...
	if len(in.GetPhrase()) > phraseSizeLimit {
		s.logger.Error("input phrase exceeds 1MB", "len(in.Phrase)", len(in.Phrase))
	...
	s.logger.Info("Norm", "result", stemsOfWords)
